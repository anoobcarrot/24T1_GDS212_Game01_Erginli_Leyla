...
public class Book : MonoBehaviour {
    ...
    // for image and text
    public Sprite[] bookPagesImages;
    public string[] bookPagesText;
    ...
    {
        // Check if the currentPage index is within bounds
        if (currentPage >= 0 && currentPage < bookPagesText.Length)
        {
            // Update right page content with TextMeshPro text
            if (RightNext != null)
            {
                // Find the TextMeshProUGUI component in the children of RightNext
                TextMeshProUGUI rightTextComponent = RightNext.GetComponentInChildren<TextMeshProUGUI>();

                // Check if the TextMeshProUGUI component exists
                if (rightTextComponent != null)
                {
                    // Update text content for the right page
                    rightTextComponent.text = bookPagesText[currentPage];
                    rightTextComponent.gameObject.SetActive(true);
                }
                else
                {
                    // Log a warning if the TextMeshProUGUI component is not found
                    Debug.LogWarning("TextMeshProUGUI component not found in children of RightNext GameObject.");
                }
            }
            else
            {
                // Log a warning if RightNext GameObject is not assigned
                Debug.LogWarning("RightNext GameObject not assigned.");
            }

            // Update left page content with TextMeshPro text
            if (LeftNext != null)
            {
                // Find the TextMeshProUGUI component in the children of LeftNext
                TextMeshProUGUI leftTextComponent = LeftNext.GetComponentInChildren<TextMeshProUGUI>();

                // Check if the TextMeshProUGUI component exists
                if (leftTextComponent != null)
                {
                    // Update text content for the left page
                    leftTextComponent.text = (currentPage > 0) ? bookPagesText[currentPage - 1] : "";
                    leftTextComponent.gameObject.SetActive(true);
                }
                else
                {
                    // Log a warning if the TextMeshProUGUI component is not found
                    Debug.LogWarning("TextMeshProUGUI component not found in children of LeftNext GameObject.");
                }
            }
            else
            {
                // Log a warning if LeftNext GameObject is not assigned
                Debug.LogWarning("LeftNext GameObject not assigned.");
            }
        }
        else
        {
            // Log a warning if currentPage index is out of bounds
            Debug.LogWarning("currentPage index out of bounds.");
        }
    }
    ...
    {
        // Check if the book is already at the last page of the page array
        if (currentPage >= bookPages.Length - 1)
        {
            // If the book is at the last page, return early without performing any action
            return;
        }

        // Perform the drag action if the book is not at the last page of the page array
        pageDragging = true;
        mode = FlipMode.RightToLeft;
        f = point;
    ...
    public void ReleasePage()
    {
        if (pageDragging)
        {
            ...
            // Check if the book has reached the last page in the array
            bool isLastPage = currentPage == (bookPages.Length - 1);

            // Prevent flipping to the next page if it's the last page in the array
            if (!isLastPage && distanceToRight < distanceToLeft && mode == FlipMode.RightToLeft)
                TweenBack();
            else if (distanceToRight > distanceToLeft && mode == FlipMode.LeftToRight && currentPage > 0)
                TweenBack();
            else
                TweenForward();
        }
